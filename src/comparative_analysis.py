...
def create_comparison_plots(summary_df):
    ...
    axes[0, 2].axhline(y=0.1, color='r', linestyle='--', alpha=0.7, label='10% threshold')
    axes[0, 2].legend()
    ...
    axes[1, 0].axhline(y=0.3, color='r', linestyle='--', alpha=0.7, label='Balance threshold')
    axes[1, 0].legend()
    
    # 5. Model performance comparison
    axes[1, 1].bar(summary_df['Dataset'], summary_df['RF_Accuracy'], alpha=0.7, label='Accuracy')
    axes[1, 1].bar(summary_df['Dataset'], summary_df['RF_F1_Score'], alpha=0.7, label='F1-Score')
