...
def create_final_report():
    ...
    overview = final_results.groupby('Category').agg({
    ...
    original_total = final_results[final_results['Category'] == 'Original']['Samples'].sum()
    ...
    print(f"• D' (cleaned): {d_prime_total:,} total samples ({(d_prime_total/original_total)*100:.1f}% retained)")
    print(f"• D'' (filtered): {d_double_total:,} total samples ({(d_double_total/original_total)*100:.1f}% retained)")
    
    # Missing values impact
    original_missing = final_results[final_results['Category'] == 'Original']['Missing_Values'].sum()
    ...
    if not model_results.empty:
        ...
        print(f"• Best performing model: {best_model['Model']} (F1: {best_model['F1_Score']:.3f})")
...
def create_comprehensive_plots(final_df, comparative_df):
    ...
    ax1.bar(sample_summary.index, sample_summary.values, color=['skyblue', 'lightgreen', 'lightcoral'])
    ...
    ax2.bar(missing_summary.index, missing_summary.values, color=['red', 'green', 'green'])
    ...
    ax4.bar(feature_summary.index, feature_summary.values, color=['orange', 'purple', 'brown'])
    ...
    if not comparative_df.empty and 'RF_Accuracy' in comparative_df.columns:
        ...
        ax6.text(0.5, 0.5, 'Model performance\ndata not available', 
    ...
    plt.suptitle('ML TP1 Comprehensive Analysis: NASA MDP Software Defect Prediction', 
